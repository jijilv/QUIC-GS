...
#   } &> "$LOG_FILE" || true
    ...
    echo "dataset=$DATASET scene=$SCENE psnr=$BEST_PSNR mem_mb=$MEM_MB time_sec=$TIME_SEC pruning_rate=$PRUNE_RATE sh_rate=$SH_RATE log=$LOG_FILE"
  done
done
