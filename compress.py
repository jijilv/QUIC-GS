...
class Pipeline:
    ...
    def run(self):
        ...
        if args.fixed_pruning_rate is not None and args.fixed_sh_rate is not None:
            ...
            stage3_time = time.time() - stage2_start
            stage2_time = 0.0  # no neighbor/refine phase in fixed-config path
            ...
            stage1_time = getattr(self.optimizer, "stage1_time", None)
            stage2_time = getattr(self.optimizer, "stage2_time", None)
            stage3_time = getattr(self.optimizer, "stage3_time", None)
        ...
        # Derive stage timings for reporting
        search_total = t3 - t2
        if stage1_time is None and stage2_time is None and stage3_time is None:
            stage1_time = search_total
            stage2_time = 0.0
            stage3_time = 0.0
        else:
            stage1_time = stage1_time if stage1_time is not None else 0.0
            stage2_time = stage2_time if stage2_time is not None else 0.0
            if stage3_time is None:
                residual = search_total - stage1_time - stage2_time
                stage3_time = max(residual, 0.0)

        print("\n" + "=" * 60)
        print("[STATS] Pipeline Time Breakdown:")
        print(f"  Load time:    {t1 - t0:>6.2f} s")
        print(f"  Score time:   {t2 - t1:>6.2f} s")
        print(f"  Stage 1 (seed/coarse) search:   {stage1_time:>6.2f} s")
        print(f"  Stage 2 (neighbor/refine):      {stage2_time:>6.2f} s")
        print(f"  Quantize time:                  {stage3_time:>6.2f} s")
        print(f"  Search total:                   {search_total:>6.2f} s")
...
if __name__ == "__main__":
    ...
    # Ensure new optional args exist even if cfg_args is old
    for attr, default in [
